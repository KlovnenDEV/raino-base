		...
		if whitelist then
			...
			if banned then
				...
				elseif ban_time > time then
					allow(
						"Você está banido | Motivo: "
							.. ban_reason
							.. " | Termina: "
							.. os.date("%d/%m/%Y - %X", ban_time)
					)
					return
				elseif ban_time < time then
					MySQL.update(
			...
			elseif message == "user data not found" then
				allow(
					"\xE2\x9D\x97[Fila] Erro: Não foi possível encontrar seu usuario no servidor do discord. Você esta no discord?"
				)
			elseif message == "user roles not found" then
				allow(
					"\xE2\x9D\x97[Fila] Erro: Não foi possível encontrar suas tags no servidor do discord. Você esta no discord?"
				)
			else
