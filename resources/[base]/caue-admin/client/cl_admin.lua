...
function Caue.Admin.CheckForSessions(self)
	if checkingForSessions then
		return
	else
		checkingForSessions = true
	end
...
function Caue.Admin.BeginAFK(self, stop)
	...
	afk.event = AddEventHandler("caue-admin:afkStringCheck", function(text)
		if string.lower(text) == afk.stringToType then
	...
	Citizen.CreateThread(function()
		...
		while true do
			...
			if curTime - beginTime >= 300000 then
				TriggerServerEvent("caue-admin:Disconnect", "AFK Kick")
				return
			end
		end
	end)
...
function Caue.Admin.AFKCheck(self)
	if afk.checkingAFK then
		return
	else
		afk.checkingAFK = true
	end
	...
	Citizen.CreateThread(function()
		while true do
			...
			if rankData and not rankData.allowafk and not inExluded then
				...
				if Vdist2(afk.posStart, curPos) <= 20.0 then
					afk.afkStart = afk.afkStart and afk.afkStart or GetGameTimer()
				else
					afk.afkStart = nil
					afk.posStart = nil
				end
...
function Caue.Admin.RunClCommand(self, cmd, args)
	if not cmd or not self:CommandExists(cmd) then
		return
	end
...
function Caue.Admin.teleportMarker(self)
	...
	if rankData and rankData.grant < 90 then
		return
	end
...
AddEventHandler("caue-admin:updateData", function(src, type, data)
	if not src or not type or not data then
		return
	end
	if not Caue._Admin.Players[src] then
		return
	end
...
AddEventHandler("caue-admin:bringPlayer", function(targPos)
	...
	Citizen.CreateThread(function()
		...
		while not HasCollisionLoadedAroundEntity(PlayerPedId()) do
			if GetGameTimer() - startedCollision > 5000 then
				break
			end
			Citizen.Wait(0)
		end
