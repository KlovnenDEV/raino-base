...
AddEventHandler("caue-jobs:changeJob", function(job, _src)
	if not job or not JOBS[job] then
		return
	end
	...
	TriggerClientEvent(
		"DoLongHudText",
		src,
		job ~= "unemployed" and "You went back to work: " .. jobName(job) or "you went out of service"
	)
end)
...
AddEventHandler("caue-jobs:paycheck", function(log, amount, _src)
	...
	TriggerClientEvent(
		"DoLongHudText",
		src,
		"A salary of  $" .. amount .. " with $" .. tax["tax"] .. " of fees were deposited in the bank."
	)
end)
...
AddEventHandler("caue-jobs:paycheckPickup", function()
	...
	if paycheck and paycheck > 0 then
		...
		TriggerClientEvent(
			"DoLongHudText",
			src,
			"Your payment of $" .. paycheck .. " has been transferred to your account."
		)
	else
