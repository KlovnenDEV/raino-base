		...
		if sell then
			...
			if ped ~= 0 and not IsPedDeadOrDying(ped) and not IsPedInAnyVehicle(ped) then
				...
				ShowFloatingHelpNotification(
					("Vou te dar " .. price .. "$ por " .. amount .. " " .. labels),
			...
			if IsPedArmed(playerPed, 7) then
				...
				if IsControlJustPressed(1, 306) then
					...
					AttachEntityToEntity(
						drug,
						playerPed,
						GetPedBoneIndex(playerPed, 64016),
						0.020,
						-0.05,
						-0.014,
						100.0,
	...
	while true do
		...
		if selldrugs then
			...
			repeat
				...
				if
					DecorGetInt(ped, "NPC_ID") == 0
					and not DecorExistOn(ped, "ScriptedPed")
				...
				then
					...
					if
						...
						and not has_value(Config.BlacklistNpc, pedRelationship)
					then
						currentped = pos

						if
							GetGameTimer() > SellCooldown
								and distance <= 3
								and not selling
								and ped ~= playerPed
								and ped ~= oldped
							or IsPedInVehicle(playerPed, vehicle, true)
								and distance <= 7
								and not selling
								and ped ~= playerPed
								and ped ~= oldped
