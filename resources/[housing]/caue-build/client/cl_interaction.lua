...
local _i, _f, _v, _r, _ri, _rf, _rl, _s, _rv, _ro, _in, _ii, _fi =
	...
	Citizen.ResultAsObject(),
	Citizen.InvokeNative,
	Citizen.PointerValueIntInitialized,
	Citizen.PointerValueFloatInitialized
...
function Build.func.buildPeeking(plan)
	...
	for i = 1, #Build.CurrentPeekModels do
		if Build.CurrentPeekModels[i] == peek.model then
			modelFound = true
			break
		end
	end
	...
	exports["caue-eye"]:AddPeekEntryByModel(
		{ peek.model },
		{ {
			event = peek.event,
			id = peek.id,
