...
function Apart.leaveApartment()
	...
	if Apart.exitPoint[Apart.currentRoomType] == nil then
		exports["caue-build"]:getModule("func").exitCurrentRoom(
			Apart.Locations[Apart.currentRoomType][Apart.currentRoomNumber]
		)
	else
...
function Apart.openHotelStash()
	local myjob = exports["caue-base"]:getChar("job")
	if
		Apart.currentRoomLockDown[Apart.currentRoomType][Apart.currentRoomNumber]
		and (not exports["caue-jobs"]:getJob(myjob, "is_police") and myjob ~= "judge")
	then
	...
	if Apart.defaultInfo[1] == Apart.currentRoomType and Apart.defaultInfo[2] == Apart.currentRoomNumber then -- owner
		...
		if
			Apart.currentRoomLockDown[Apart.currentRoomType][Apart.currentRoomNumber]
			and (exports["caue-jobs"]:getJob(myjob, "is_police") or myjob == "judge")
		then
			Apart.OpenStash()
		end
	end
...
AddEventHandler("apartment:attemptEntry", function(roomNumberSent)
	...
	if not roomNumber and not roomType then
		return
	end
...
function Apart.func.upgradeApartment(apartmentTargetType)
	if apartmentTargetType <= Apart.defaultInfo[1] then
		return false, "Cannot downgrade"
	end
	if apartmentTargetType > #Apart.info then
		return false, "Not a valid Apartment"
	end
	local apartmentInfo = RPC.execute("getApartmentInformation")
	if apartmentInfo == nil then
		return false, "Failed to gather info"
	end

	local hasBankAccount, bankAccountId = RPC.execute("GetDefaultBankAccount", exports["caue-base"]:getChar("id"))

	if hasBankAccount then
		local comment = "Upgraded from apartment type ["
			.. Apart.defaultInfo[1]
			.. "] to apartment type ["
			.. apartmentTargetType
			.. "]"
		local success, message = RPC.execute(
			"DoTransaction",
			bankAccountId,
			1,
			apartmentInfo[apartmentTargetType].apartmentPrice,
			comment,
			1,
			1
		)
